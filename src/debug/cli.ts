 ...
 *   source:ui5changelog          Collect SAPUI5 changelog entries for the week
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "source:zeis":
      ...
      break;
    case "source:ui5changelog":
      out("UI5 Changelog", await collectUi5Changelog(range));
      break;
    case "collect":
    ...
    case "ai:article": {
      ...
      if (!run)
        throw new Error("No saved run found. Run `npm run collect` first.");
    ...
    case "ai:linkedin": {
      ...
      out(
        "LinkedIn post",
        await writeLinkedIn(fakeArticle, "https://example.com/posts/test/"),
      );
      break;
    }
    case "ai:thumbnail": {
...
main().catch((err) => {
  log.error(`Debug failed: ${err instanceof Error ? err.stack : String(err)}`);
  process.exit(1);
});
